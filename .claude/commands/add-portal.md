...
desk:
  id: add-portal
  invocation: /add-portal
  title: Add Portal
  description: Generate a job-portal search skill.
  arguments:
    - kind: url
      name: url
      required: false
...
4. **Check access requirements and terms.**
   ...
   - If robots.txt disallows the paths or the portal's terms prohibit scraping or other automated access, tell the user plainly and **stop unless they explicitly accept the risk**. If they do, scaffold the skill with `enabled: false` and a warning in the same tone as `linkedin-search` (off by default, their risk if they set `enabled: true`, low volume, no commercial or bulk use). Do not set `enabled: true` for them. The shipped default must stay off.
...
- **`package.json`:** name `<portal>-cli`, `"type": "module"`, scripts `start`, `test` (`bun test --timeout 30000`), and `typecheck` (`tsc --noEmit`); dev-only dependencies in the zero-dependency default. The TypeScript devDependency is `^5.8.0`. For a zero-dependency CLI, `start` is `node --experimental-strip-types src/cli.ts`, which is the command CI runs. If the CLI depends on a Bun-only library such as `@bunli/core`, `start` stays `bun run src/cli.ts` and CI starts it with Bun.
- **`tsconfig.json`:** `"erasableSyntaxOnly": true`. Node's strip-only type stripping rejects parameter properties, enums, namespaces, and decorators. Plain `tsc` and Bun accept those, so without this flag a CLI that cannot start still typechecks. The flag is unknown before TypeScript 5.8, which is why the version floor above ships with it. Copy this from `linkedin-search`. Relative imports use the `.ts` extension (`./helpers.ts`). Node does not map a `.js` specifier onto a `.ts` file, so a `.js` import makes `node --experimental-strip-types src/cli.ts --help` fail before the program starts.
